...
// app.get("/get-data-for-is-present",(req,res)=>{

//     const selectQuery = "SELECT present_time FROM payslip";
...
cron.schedule("0 0 1 * *", () => {
  ...
  db.query("SHOW COLUMNS FROM payslip", (err, columns) => {
    if (err) return console.log("Database Error");
    const exists = columns.some(col => col.Field === columnName);
    if (!exists) {
      db.query(`ALTER TABLE payslip ADD ${columnName} INT DEFAULT 0`, (err) => {
        if (err) return console.log("Database Error");
        console.log("Next Month Field Added");
      });
    }
...
app.post('/mark-attendance', (req, res) => {
    ...
    db.query(`SELECT ${columnName}, present_time FROM payslip WHERE emp_email = ?`, [email], (err, info) => {
      if (err) return res.status(400).send({ message: "Database Error" });
      if (info.length === 0) return res.status(404).json({ message: "Employee not found" });
  
      const currentCount = info[0][columnName] || 0;
      const storedDateOnly = new Date(info[0].present_time).toISOString().split('T')[0]; 
      const currentDateOnly = new Date(date).toISOString().split('T')[0]; 
        
      console.log("Current Date", currentDateOnly);
      console.log("Stored Date", storedDateOnly);
      
      if (currentDateOnly === storedDateOnly) {
        return res.status(400).send({ message: "Already Present Today" }); 
      }
  
      if (status === 1) {
        db.query(`UPDATE payslip SET ${columnName} = ?, present_time = ? WHERE emp_email = ?`,
          [currentCount + 1, date, email], (err) => {
            if (err) return res.status(400).send({ message: "Database Error" });
            return res.status(200).send({ message: "Present Added" });
          });
      } else {
        return res.status(200).send({ message: "Absent recorded" });
      }
    });
  });
  
app.get("/get-data-for-is-present/:email", (req, res) => {
  const { email } = req.params;
  db.query("SELECT present_time FROM payslip WHERE emp_email = ?", [email], (err, info) => {
    if (err) return res.status(400).send({ message: "Database Error" });
    console.log(info);
...
app.get("/get-user-for-attendance/:email", (req, res) => {
  const { email } = req.params;
  db.query("SELECT emp_id, emp_name FROM payslip WHERE emp_email = ?", [email], (err, info) => {
    if (err) return res.status(400).send({ message: "Database Error" });
    return res.status(200).send(info);
  });
});
...
app.post("/attendance-absent", (req, res) => {
  ...
  transporter.sendMail(mailOptions, (err) => {
    if (err) return res.status(400).send({ message: "Email Error" });
    return res.status(200).send({ message: "Leave Applied" });
  });
});
